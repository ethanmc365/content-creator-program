...
//
// A ticket in two parts torn along a perforation: the brand stub carries WHAT it
// is (the amount, where it came from), the white half carries the CODE, which is
// the only thing on it anybody will copy. A used voucher greys out and is
// stamped rather than removed - it is still a record of something earned.
//
...
export default function VoucherTicket({ reward, onToggleUsed, busy = false }) {
  ...
  return (
    <div
      className={cx(
        'relative flex overflow-hidden rounded-card shadow-card transition-[filter,opacity] duration-300',
        used && 'opacity-70 grayscale',
      )}
    >
      {/* The stub: what it is worth and where it came from. */}
      <div className="relative flex w-[28%] min-w-[6rem] shrink-0 flex-col justify-between bg-gradient-to-br from-brand to-brand-light p-3.5 text-white sm:w-[32%] sm:min-w-[7.5rem] sm:p-5">
      ...
      {/* The perforation: two notches bitten out of the edges and a dashed
          tear line between them. The notches are page-coloured circles, so
          they read as holes on white and on the dark theme alike. */}
      <div aria-hidden className="relative w-0">
        <span className="ticket-notch absolute -left-2.5 -top-2.5 h-5 w-5 rounded-full" />
        <span className="ticket-notch absolute -bottom-2.5 -left-2.5 h-5 w-5 rounded-full" />
        <span className="absolute inset-y-3 left-0 border-l-2 border-dashed border-gray-200" />
      </div>

      {/* The code. */}
      <div className="flex min-w-0 flex-1 flex-col justify-between gap-3 bg-white p-4 sm:p-5">
      ...
      {used && (
        <span
          aria-hidden
          className="pointer-events-none absolute bottom-3.5 left-3.5 rotate-[-8deg] rounded-md border-2 border-white px-2 py-0.5 text-[11px] font-extrabold uppercase tracking-widest text-white sm:left-5"
        >
          {tr('Used')}
        </span>
      )}
